...
function actualizarResumen(lista) {
  ...
  totalActivos.textContent = lista.filter((u) => u.estado === "ACTIVO").length;
  totalInactivos.textContent = lista.filter((u) => u.estado === "INACTIVO").length;
}
...
function abrirModalEditar(idUsuario) {
  ...
  const idRol = rolesPorNombre[usuarioSeleccionado.rol] || 3;
  editRol.value = idRol;
